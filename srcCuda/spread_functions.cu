...
__global__ void calculate_spread_probabilities(
    ...
    for (int n = 0; n < 8; n++) {
        ...
        float slope_term = sin(atan((neighbor.elevation - burning_cell.elevation) / distance));
        float wind_term = cos(angles[n] - burning_cell.wind_direction);
        float elev_term = (neighbor.elevation - elevation_mean) / elevation_sd;
        float linpred = params->independent_pred;

        if (neighbor.vegetation_type == SUBALPINE) {
            linpred += params->subalpine_pred;
        } else if (neighbor.vegetation_type == WET) {
            linpred += params->wet_pred;
        } else if (neighbor.vegetation_type == DRY) {
            linpred += params->dry_pred;
        }

        linpred += params->fwi_pred * neighbor.fwi;
        linpred += params->aspect_pred * neighbor.aspect;
        linpred += wind_term * params->wind_pred + elev_term * params->elevation_pred +
                  slope_term * params->slope_pred;
